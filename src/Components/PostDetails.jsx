...
import { postDetailMorePostQuery, postDetailQuery } from '../utilityFunctions/data';
  ...
  const fetchPostDetails = () => {
    ...
    if (query) {
      client.fetch(`${query}`).then((data) => {
        ...
        //console.log(data);
        if (data[0]) {
          const newQuery = postDetailMorePostQuery(data[0]);
    ...
    <>
      {postDetail && (
        <div className="flex xl:flex-row flex-col m-auto bg-white" style={{ maxWidth: '1500px', borderRadius: '32px' }}>
          ...
          <Link to={`/user-profile/${postDetail?.postedBy._id}`} className="flex gap-2 mb-5 mt-1 items-center bg-white rounded-lg self-center ">
          ...
          <div className="w-full p-5 flex-1 xl:min-w-620">
            <div className="flex items-center justify-between">
              ...
              <a href={postDetail.home_url} target="_blank" rel="noreferrer">
                {postDetail.home_url?.slice(8)}
              </a>
            </div>
            <div>
            ...
            <h2 className="mt-10 text-2xl self-center text-center">Comments:</h2>
